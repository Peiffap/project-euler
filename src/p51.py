...
while True:
    ...
    if root and root[-1] in ['1', '3', '7', '9'] and root.count('*') != 0 and root.count('*') % 3 == 0: # if last digit is even or 5
        #print(root)
        cnt = 0 # count number of primes in generated family
        family = [root.replace('*', str(i)) for i in range(10)] # replace *s
        for f in family:
            if f[0] != '0' and sieve[int(f)]: # can't have a leading zero and needs to be prime
                cnt += 1
            ...
            if best_cnt >= 8:
                for x in family: # print the smallest prime in the family
                    if sieve[int(x)]:
                        ...
                        print(end - start)
                        quit()
    for c in [str(i) for i in range(10)] + ['*']: # generate new possibilities to add to queue
