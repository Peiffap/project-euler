...
while True:  # grid width
    ...
    for h in range(
        1, w + 1
    ):  # grid height (without loss of generality less than or equal to width)
        s = 0
        if h == 1:
            ...
            for rw in range(1, w + 1):  # rectangle width
                for rh in range(1, h + 1):  # rectangle height
                    s += (w + 1 - rw) * (
                        h + 1 - rh
                    )  # as many (rw, rh) rectangles as positions for top left corner
        ...
        if (
            os >= goal
        ):  # if we are above 2000000, making the grid higher will never be better
            break
