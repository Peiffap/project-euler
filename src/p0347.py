...
for i in range(len(ps) - 1):  # loop over the smallest prime p
    ...
    if (
        p * p > lim
    ):  # since p < q, if p * p > lim, so does p * q, and we don't need to increase p further
        break
    for j in range(i + 1, len(ps)):  # wlog (since we only consider unique maxes), p < q
        ...
        while (
            p * q**qexp <= lim
        ):  # for each possible exponent of q (looping while the minimal product is below lim)
            pexp = int(
                math.log(lim / (q**qexp), p)
            )  # compute the maximal exponent of p that still yields a valid product
            m = max(
                m, q**qexp * p**pexp
            )  # keep track of the maximal value among optima
            qexp += 1
