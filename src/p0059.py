...
common_words = ['I', 'the', 'The', 'be', 'Be', 'to', 'To', 'of', 'Of', 'and', 'And', 'a', 'A', 'in', 'In',
'that', 'That', 'have', 'Have', 'it', 'It', 'for', 'For', 'not', 'Not', 'on', 'On' 'with', 'With']
...
for i in range(26):
    for j in range(26):
        for k in range(26):
            key = chr(ord('a') + i) + chr(ord('a') + j) + chr(ord('a') + k) # loop over all possible length-3 lowercase keys
            #print(key)
            ns = decrypt(s, key) # decrypt and obtain new string
            if cnt_words(ns, common_words) > 10: # count common words, 10 is a random ~1/3 threshold
