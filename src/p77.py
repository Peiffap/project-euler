...
Pn = [[(p, p) for p in pl]] # Pn[0] = all possible (s = p_1 + ... + p_{i+1}, p_1 < ... < p_{i+1}) tuples such that s <= limit
i = 0
while True:
    nps = []
    for j in Pn[i]: # for each tuple of size i
        for k in Pn[0][pindices[j[-1]] + 1:]: # for each prime p_{i+1} > p_i > ... > p_1
            if j[0] + k[-1] <= limit: # if s = p_1 + ... + p_{i + 1} <= limit
                nps.append(tuple([j[0] + k[-1]] + list(j[1:]) + [k[-1]])) # add to tuples for size i + 1
            else:
                break
...
def pp(n):
    if n == 0: # base case - the case n < 0 is handled by checking we never subtract more from n than n
        return 1
    ppn = 0
    sign = 1
