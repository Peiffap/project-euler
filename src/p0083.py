...
fo = open("../data/matrix83.txt", "r")  # open input file
f = list(fo)
fo.close()

grid = [
    [int(i) for i in line.strip().split(",")] for line in f
...
while (
    ...
    if i + 1 < len(grid):  # if we can move down
        q.append(
            (s + grid[i + 1][j], i + 1, j, path + [(i + 1, j)])
        )  # add result of moving down to the queue
    if i - 1 >= 0:  # if we can move up
        q.append(
            (s + grid[i - 1][j], i - 1, j, path + [(i - 1, j)])
        )  # add result of moving up to the queue
    if j + 1 < len(grid):  # if we can move right
        q.append(
            (s + grid[i][j + 1], i, j + 1, path + [(i, j + 1)])
        )  # add result of moving right to the queue
    if j - 1 >= 0:  # if we can move left
        q.append(
            (s + grid[i][j - 1], i, j - 1, path + [(i, j - 1)])
        )  # add result of moving left to the queue
    for e in q:
        if (e[1], e[2]) in seen:
            q.remove(
                e
            )  # remove node (i, j) from the queue if it has already been reached with a lower path sum
