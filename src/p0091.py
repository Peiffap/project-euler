...
for x1, y1, x2, y2 in prod: # for all coordinates
    ...
    if not (x1 == 0 and x2 == 0) and not (y1 == 0 and y2 == 0) and not (x1 == 0 and y1 == 0) and not (x2 == 0 and y2 == 0) and not (x1 == x2 and y1 == y2):
        edges = [x1 ** 2 + y1 ** 2, x2 ** 2 + y2 ** 2, (x2 - x1) ** 2 + (y2 - y1) ** 2] # compute edge lengths
        if sum(edges) == 2 * max(edges): # a triangle has a right angle iff a^2 + b^2 = c^2, or if a^2 + b^2 + c^2 = 2c^2
            cnt += 1
