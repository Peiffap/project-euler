...
for i in range(1, 10000):  # smallest consecutive square can't be larger than 1e4 (sum can only go up to 1e8)
    #print(i)
    for j in range(i + 1, 10001):  # largest consecutive square must be strictly larger than smallest
        ssq = sum(squares[i:j + 1])  # compute sum of consecutive squares from i to j (inclusive)
