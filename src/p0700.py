...
while v > 1.6e7:  # start by bringing down the upper bound through brute-force
    ...
    if (
        v < smallest
    ):  # if a smaller eulercoin has been found, add to sum and update smallest known
        smallest = v
        s += v
        # print(n, smallest, s)
...
for i in range(1, v):  # for the remaining values after brute-forcing
    ...
    if (
        td <= lowest_iter
    ):  # this value is the smallest to have been reached in its minimum number of iterations, and is thus an eulercoin
        lowest_iter = td
        # print(i, td)
        s += i
