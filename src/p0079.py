...
def shortest_common_supersequence(s1, s2):
    ...
    for i in range(len(s1) + 1):
        ...
        for j in range(len(s2) + 1):
            ...
            if i == 0:  # if nothing in s1, scs is s2[:j]
                ...
                s1[i - 1] == s2[j - 1]
            ):  # if both have the same last character c, scs is scs(s1[:i-1], s2[:j-1]) + c
                dp[i][j] = dp[i - 1][j - 1] + 1
    ...
    while i > 0 and j > 0:  # while we have to add parts of both strings still
        if s1[i - 1] == s2[j - 1]:  # if the last character is the same, add it
            ...
            dp[i - 1][j] > dp[i][j - 1]
        ):  # otherwise add the character that leads to the shortest common supersequence
            s += s2[j - 1]
            j -= 1
        else:
            s += s1[i - 1]
...
s = []
for line in f:
    s.append(line.strip())  # read the input
...
for p in range(len(s)):  # loop over all possible permutations of the input
    ...
    while (
        sp
    ):  # find a greedy shortest common supersequence of all passcodes in this order
        minlen = 151
        for si in sp:
            nscs = shortest_common_supersequence(
                shortest, si
            )  # compute the scs over a subset of passcodes
            if (
                len(nscs) < minlen
            ):  # if better than the best known for this order of passcodes, replace the best known
                minlen = len(nscs)
                nshortest = nscs
                tr = si
        shortest = nshortest
        sp.remove(tr)
    if rshortest == "" or int(shortest) < int(
        rshortest
    ):  # update the order-independent scs
        rshortest = shortest
